Ext.define('iFlat.view.pam.MemberProb', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.pam-memberprob',
    xtype: 'pam-memberprob',

    requires: [
        'iFlat.view.pam.MemberProbController'
    ],

    controller: 'pam-memberprob',
    store: pamMemberProbStore = Ext.create('iFlat.store.pam.Member', {
        proxy: {
            extraParams: {
                'pamMember.type': '预备',
            }
            ...
            Ext.Ajax.request({
                ...
                success: function (response, opts) {
                    ...
                    if (!Flat.util.isEmpty(info) && info.length > 0) {
                        ...
                        pamMemberProbStore.getProxy().extraParams['pamMember.pbName'] = pbName;
                        pamMemberProbStore.reload();
                    } else {
                        Ext.getCmp('pam-memberprob-add').hide();
                        Ext.Msg.show({
                            title:'提示',
    ...
    dockedItems: [{
        ...
        items: [{
            text: '新增',
            id: 'pam-memberprob-add',
            ui: 'orig-blue',
            handler: 'showMemberEdit',
        }, '->', {
            text: '刷新',
            id: 'pam-memberprob-refresh',
            handler: 'refreshList',
        }],
    }],
    columns: [{
        ...
        header: '转正时间',
        dataIndex: 'pamMember.becomeProbMemberProb',
        formatter: 'date("Y-m-d")',
        width: 120
    }, {
        header: '工作时间',
