Ext.define('iFlat.view.xr.SrWorkshopSettlementApproveController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.xr-srworkshopsettlementapprove',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        srWorkshopSettlementApproveDetailStore.getProxy().extraParams['srSettlementDetl.pid'] = id;
                        srWorkshopSettlementApproveDetailStore.reload({
                            callback: function (records, option, success) {
                                var amount = 0;
                                for (var i = 0; i < records.length; i++) {
                                    amount += records[i].get('srSettlementDetl.amountSecond');
                                }
                                Ext.getCmp('xr-srworkshopsettlementapprove-sum').setValue(amount);
                            }
                        });
                    }
                }
    ...
    completeTask: function (btn) {
        var panel = btn.up('xr-srworkshopsettlementapprove');
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-srworkshopsettlementapprove-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-srworkshopsettlementapprove-down2');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
