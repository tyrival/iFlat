Ext.define('iFlat.view.xr.TrHrAuditController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.xr-trhraudit',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        trHrAuditDetailStore.getProxy().extraParams['trSettlementDetl.pid'] = id;
                        trHrAuditDetailStore.reload({
                            callback: function (records, option, success) {
                                var amount = 0;
                                for (var i = 0; i < records.length; i++) {
                                    amount += records[i].get('trSettlementDetl.amountSecond');
                                }
                                Ext.getCmp('xr-trhraudit-sum').setValue(amount);
                            }
                        });
                    }
                }
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-trhraudit-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-trhraudit-down2');
         btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
