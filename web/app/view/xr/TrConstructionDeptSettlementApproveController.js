Ext.define('iFlat.view.xr.TrConstructionDeptSettlementApproveController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.xr-trconstructiondeptsettlementapprove',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        trConstructionDeptSettlementApproveDetailStore.getProxy().extraParams['trSettlementDetl.pid'] = id;
                        trConstructionDeptSettlementApproveDetailStore.reload({
                            callback: function (records, option, success) {
                                var amount = 0;
                                for (var i = 0; i < records.length; i++) {
                                    amount += records[i].get('trSettlementDetl.amountSecond');
                                }
                                Ext.getCmp('xr-trconstructiondeptsettlementapprove-sum').setValue(amount);
                            }
                        });
                    }
                }
    ...
    completeTask: function (btn) {
        var panel = btn.up('xr-trconstructiondeptsettlementapprove');
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-trconstructiondeptsettlementapprove-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-trconstructiondeptsettlementapprove-down2');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
