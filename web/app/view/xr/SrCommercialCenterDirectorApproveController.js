Ext.define('iFlat.view.xr.SrCommercialCenterDirectorApproveController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.xr-srcommercialcenterdirectorapprove',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        srCommercialCenterDirectorApproveDetailStore.getProxy().extraParams['srSettlementDetl.pid'] = id;
                        srCommercialCenterDirectorApproveDetailStore.reload({
                            callback: function (records, option, success) {
                                var amount = 0;
                                for (var i = 0; i < records.length; i++) {
                                    amount += records[i].get('srSettlementDetl.amountFirst');
                                }
                                Ext.getCmp('xr-srcommercialcenterdirectorapprove-sum').setValue(amount);
                            }
                        });
                    }
                }
    ...
    completeTask: function (btn) {
        var panel = btn.up('xr-srcommercialcenterdirectorapprove');
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('xr-srcommercialcenterdirectorapprove-down');
