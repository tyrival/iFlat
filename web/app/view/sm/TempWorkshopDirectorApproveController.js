Ext.define('iFlat.view.sm.TempWorkshopDirectorApproveController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.sm-tempworkshopdirectorapprove',
        ...
        Ext.Ajax.request({
            ...
            success: function(response, opts) {
                ...
                if (!Flat.util.isEmpty(id)) {
                    smTemporaryDetailStore.getProxy().extraParams['temporaryDetail.pid'] = id;
    ...
    showComment: function (btn) {
        ...
        win.down('grid').setStore(Ext.create('iFlat.store.workflow.Comment', {
            proxy: {
                ...
                extraParams: {
                    'temporary.id': btn.up('window').down('textfield[name=temporary.id]').getValue()
                }
            }
        }))
        win.show();
