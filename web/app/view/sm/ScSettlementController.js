Ext.define('iFlat.view.sm.ScSettlementController', {
    ...
    edit: function (grid, rowIndex, colIndex, item, e, record, row) {
        ...
        if (record.get('scSettlement.status') != '未提交') {
            ...
            Ext.getCmp('sm-scsettlementedit-time').setEditable(false);
            Ext.getCmp('sm-scsettlementedit-projno').setEditable(false);
            ...
            Ext.getCmp('sm-scsettlementedit-time').setEditable(true);
            Ext.getCmp('sm-scsettlementedit-projno').setEditable(true);
    ...
    onProjNoChange: function (combo, record, eOpts) {
        Ext.getCmp('sm-scsettlementedit-projname').setValue(record.get('rptProject.name'));
    },
    ...
    uploadFile: function(btn) {
        ...
        if (form.isValid()) {
            form.submit({
                url: 'sm_importScSettlement.action',
                method: 'POST',
                waitMsg: '正在导入......',
                success: function (fp, o) {
                    Flat.util.tip(o.response.responseText);
    ...
    downloadTemplate: function(btn) {
        Ext.Ajax.request({
            url: 'sm_templateScSettlement.action',
            method: 'post',
            success: function(response, opts) {
