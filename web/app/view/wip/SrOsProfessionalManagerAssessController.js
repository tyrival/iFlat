Ext.define('iFlat.view.wip.SrOsProfessionalManagerAssessController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.wip-srosprofessionalmanagerassess',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        var id = record.get('srOutsource.id');
                        srOsProfessionalManagerAssessDetailStore.getProxy().extraParams['srOutsourceDetl.pid'] = id;
                        srOsProfessionalManagerAssessDetailStore.reload();
                    }
                }
            }
        })
    ...
    completeTask: function (btn) {
        var panel = btn.up('wip-srosprofessionalmanagerassess');
        ...
        if (text == 'pass') {
            url = 'wip_approveSrOutsourceWithAssess.action';
        }
        var canSubmit = false;
    ...
    showBidding: function () {
        var id = Ext.getCmp('wip-srosprofessionalmanagerassess-id').getValue();
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprofessionalmanagerassess-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprofessionalmanagerassess-down2');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange3: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprofessionalmanagerassess-down3');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange4: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprofessionalmanagerassess-down4');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange5: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprofessionalmanagerassess-down5');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
