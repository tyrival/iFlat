Ext.define('iFlat.view.wip.SrOsProjectManagerAssessController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.wip-srosprojectmanagerassess',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        var id = record.get('srOutsource.id');
                        srOsProjectManagerAssessDetailStore.getProxy().extraParams['srOutsourceDetl.pid'] = id;
                        srOsProjectManagerAssessDetailStore.reload();
                    }
                }
            }
        })
    ...
    completeTask: function (btn) {
        var panel = btn.up('wip-srosprojectmanagerassess');
    ...
    showBidding: function () {
        var id = Ext.getCmp('wip-srosprojectmanagerassess-id').getValue();
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprojectmanagerassess-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprojectmanagerassess-down2');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange3: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprojectmanagerassess-down3');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange4: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprojectmanagerassess-down4');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange5: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srosprojectmanagerassess-down5');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
