Ext.define('iFlat.view.wip.SrOsSettlementApproveController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.wip-srossettlementapprove',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        var id = record.get('srOutsource.id');
                        srOsSettlementApproveDetailStore.getProxy().extraParams['srOutsourceDetl.pid'] = id;
                        srOsSettlementApproveDetailStore.reload();
                    }
                }
            }
        })
    ...
    completeTask: function (btn) {
        var panel = btn.up('wip-srossettlementapprove');
    ...
    showBidding: function () {
        var id = Ext.getCmp('wip-srossettlementapprove-id').getValue();
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossettlementapprove-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossettlementapprove-down2');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange3: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossettlementapprove-down3');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange4: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossettlementapprove-down4');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange5: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossettlementapprove-down5');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
