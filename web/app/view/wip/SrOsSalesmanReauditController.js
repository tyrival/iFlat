Ext.define('iFlat.view.wip.SrOsSalesmanReauditController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.wip-srossalesmanreaudit',
            ...
            callback: function(records, option, success) {
                ...
                if (!success) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        var id = record.get('srOutsource.id');
                        srOsSalesmanReauditDetailStore.getProxy().extraParams['srOutsourceDetl.pid'] = id;
                        srOsSalesmanReauditDetailStore.reload();
                    }
                }
            }
        })
    ...
    completeTask: function (btn) {
        var panel = btn.up('wip-srossalesmanreaudit');
    ...
    showBidding: function () {
        var id = Ext.getCmp('wip-srossalesmanreaudit-id').getValue();
    ...
    onAttachmentChange: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossalesmanreaudit-down');
    ...
    onAttachmentChange2: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossalesmanreaudit-down2');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange3: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossalesmanreaudit-down3');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange4: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossalesmanreaudit-down4');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
    ...
    onAttachmentChange5: function(field, newValue, oldValue, eOpts) {
        var btnDown = Ext.getCmp('wip-srossalesmanreaudit-down5');
        btnDown.setHref(newValue);
        if (!Flat.util.isEmpty(newValue)) {
            btnDown.show();
        } else {
